...
def conv_factory_RProj(x, nb_filter, DenseLayer, Conv2DLayer, BNLayer, dropout_rate=None, weight_decay=1E-4):
    """Apply BatchNorm, Relu 3x3Conv2D, optional dropout
    :param x: Input keras network
    :param nb_filter: int -- number of filters
    :param dropout_rate: int -- dropout rate
    ...
    x = BNLayer(axis=1,
               ...
               beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = Conv2DLayer(nb_filter,(3, 3), strides=(1, 1),
...
def transition_RProj(x, nb_filter, DenseLayer, Conv2DLayer, BNLayer, dropout_rate=None, weight_decay=1E-4):
    """Apply BatchNorm, Relu 1x1Conv2D, optional dropout and Maxpooling2D
    :param x: keras model
    :param nb_filter: int -- number of filters
    :param dropout_rate: int -- dropout rate
    ...
    x = BNLayer(axis=1,
               ...
               beta_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = Conv2DLayer(nb_filter, (1, 1), strides=(1, 1),
                      kernel_initializer="he_uniform",
                      padding="same",
                      use_bias=False,
...
def denseblock_RProj(x, nb_layers, nb_filter, growth_rate,DenseLayer, Conv2DLayer, BNLayer,
               dropout_rate=None, weight_decay=1E-4):
    """Build a denseblock where the output of each
       conv_factory is fed to subsequent ones
    :param x: keras model
    ...
    for i in range(nb_layers):
        # embed()
        x = conv_factory_RProj(x, growth_rate, DenseLayer, Conv2DLayer, BNLayer, dropout_rate, weight_decay)
        list_feat.append(x)
        x = merge(list_feat, mode='concat', concat_axis=concat_axis)
        nb_filter += growth_rate
